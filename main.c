...
int main(int nargs, char *argv[]){
  ...
  int i,j,k,x,l,numInts=512,verbose=0,deg=0,M,job=0,inner_itermax=10,outer_itermax=10;
  double *r , *t , *qKnownList  , *qs,*qa,*grad;
  ...
  int     line_search_itermax=3;   /* steps in line search        */
...
for (k=0;k<numInts+1;k++){
  ...
  for (i=0; i<outer_itermax; i++) {     
    ...
    for (j=0; j<inner_itermax; j++) {  
      ...
      for (k=0; k<line_search_itermax; k++) {   /* boundary update */
      ...
      if (file == NULL) {  
        ...
        for (l=0; l<=deg+1; l++) fprintf(file,"\n%g",a_new[l]);
        fprintf(file,"\n];\n");
        fclose(file);
      }
